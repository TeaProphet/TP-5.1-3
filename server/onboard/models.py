...
class User(models.Model):
	...
	def auth_firebase_token(self, database: firebase.Database, auth: firebase.Auth):
		auth.sign_in_with_custom_token(self.idToken)
	...
	def save_firebase(self, database: firebase.Database, auth: firebase.Auth):
		...
		database.child("users").set({self.login: {"password": self.password, "idToken": self.idToken, "reputation": 1,
		"age": self.age, "games": self.games, "vk": self.vk, "tg": self.tg, "played_sessions": self.played_sessions}})
...
class UserSerializer(serializers.Serializer):
	...
	reputation = serializers.IntegerField(default=None)
